...
async def chat(request: ChatRequest, req: Request):
    ...
    if cid not in _history_store:
        _history_store[cid] = {"messages": [], "title": "", "_owner_type": st, "_owner_id": sid}
...
async def rag_upload(file: UploadFile = File(...)):
    ...
    fp = UPLOAD_DIR / f"{uuid.uuid4().hex}_{file.filename}"
    content = await file.read(); fp.write_bytes(content)
    try:
        rag = RAGPipeline()
        chunks = await rag.ingest_file(str(fp))
        return FileUploadResponse(filename=file.filename, chunks=chunks, status="ok")
    except Exception as e:
...
async def get_history(req: Request, device_id: str | None = Query(None)):
    ...
    for cid, entry in scoped.items():
        ...
        conversations.append(HistoryItem(id=cid, title=title, created_at=datetime.now(timezone.utc).isoformat(), message_count=len(msgs)))
...
async def migrate_conversations(body: MigrateRequest, req: Request):
    ...
    for cid, entry in _history_store.items():
        if isinstance(entry, dict) and entry.get("_owner_type") == "device" and entry.get("_owner_id") == body.device_id:
